...
import { calculateCommissionSnapshot } from './payments/money.js';
import { refreshDriverCommissionStanding } from './commissionService.js';
...
export const createBooking = async ({ userId, customerName, driverId, destination, startDate: startValue, endDate: endValue, partySize = 1, notes = '', itineraryId }) => {
  await refreshDriverCommissionStanding(driverId);
  ...
  try {
    await session.withTransaction(async () => {
      const driver = await Driver.findOne({ _id: driverId, availability: true, verificationStatus: 'verified', identityVerificationRequired: false, commissionStanding: { $ne: 'restricted' }, accountStatus: 'active' }).session(session);
      ...
      const money = calculateCommissionSnapshot({ dailyRate: dailyRateAtBooking, numberOfDays });
      [createdBooking] = await Booking.create([{
        ...
        currency: money.currency, driverDailyRateAtBooking: money.driverDailyRate, driverSubtotal: money.agreedTourPrice,
        platformCommissionRateBps: money.commissionRateBps, platformCommissionAmount: money.commissionAmount, travelerTotal: money.agreedTourPrice, paymentStatus: 'unpaid',
        agreedTourPrice:money.agreedTourPrice,commissionRateBps:money.commissionRateBps,commissionAmount:money.commissionAmount,tourPaymentArrangement:'direct_to_driver',
      }], { session });
