...
const emptyDriver = { name: '', phoneNumber: '', vehicleType: '', vehicleModel: '', vehicleCapacity: 4, dailyRate: 0, yearsOfExperience: 0, languages: '', serviceAreas: '', bio: '', availability: true, verificationStatus: 'verified', image: null };
...
export default function Admin() {
  ...
  const openDriver = (driver = null) => { setError(''); setDriverForm(driver ? { ...driver, name: driver.fullName || driver.name, languages: (driver.languages || []).join(', '), serviceAreas: (driver.serviceAreas || []).join(', '), image: null } : emptyDriver); setModal({ type: 'driver', id: driver?._id }); };
  ...
  const saveDriver = async (event) => {
    event.preventDefault(); setSaving(true); setError('');
    const payload = new FormData();
    ['name', 'phoneNumber', 'vehicleType', 'vehicleModel', 'vehicleCapacity', 'dailyRate', 'yearsOfExperience', 'languages', 'serviceAreas', 'bio', 'availability', 'verificationStatus'].forEach((field) => payload.append(field, driverForm[field]));
  ...
  const filteredDrivers = drivers.filter((driver) => `${driver.fullName || driver.name} ${driver.vehicleType}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="min-h-screen bg-slate-100/70">
    ...
    <div className="page-shell py-8"><div className="mb-8 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2">{tabs.map((item) => <button key={item.id} onClick={() => { setTab(item.id); setQuery(''); }} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold ${tab === item.id ? 'bg-teal-800 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}><item.icon size={17} /> {item.label}</button>)}</div>
      ...
      {loading ? <div className="grid gap-5 md:grid-cols-3">{[1,2,3].map((n) => <div key={n} className="h-36 animate-pulse rounded-3xl bg-slate-200" />)}</div> : <>
        ...
        {tab === 'drivers' && <section><ManagementHeader title="Driver network" count={drivers.length} query={query} setQuery={setQuery} button="Add driver" onAdd={() => openDriver()} /><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredDrivers.map((driver) => { const name = driver.fullName || driver.name; return <article key={driver._id} className="card overflow-hidden"><div className="relative"><img src={getImageUrl(driver.profileImage || driver.image)} alt={name} className="h-48 w-full object-cover" /><span className={`absolute right-4 top-4 rounded-full px-3 py-1 text-xs font-bold ${driver.verificationStatus === 'verified' && driver.availability ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}>{driver.verificationStatus === 'verified' ? (driver.availability ? 'Active' : 'Unavailable') : driver.verificationStatus}</span></div><div className="p-5"><h3 className="font-serif text-xl font-bold">{name}</h3><p className="mt-1 text-sm text-slate-500">{driver.vehicleModel || driver.vehicleType} · LKR {Number(driver.dailyRate || 0).toLocaleString()}/day</p><Actions onEdit={() => openDriver(driver)} onDelete={() => remove('drivers', driver)} /></div></article>; })}</div><NoResults items={filteredDrivers} /></section>}
    ...
    {modal?.type === 'driver' && <AdminModal title={modal.id ? 'Edit driver' : 'Add driver'} onClose={() => setModal(null)}>{error && <ModalError message={error} />}<form onSubmit={saveDriver} className="space-y-4"><FormField label="Full name" value={driverForm.name} onChange={(value) => setDriverForm({ ...driverForm, name: value })} placeholder="Driver name" /><div className="grid gap-4 sm:grid-cols-2"><FormField label="Phone" value={driverForm.phoneNumber} onChange={(value) => setDriverForm({ ...driverForm, phoneNumber: value })} placeholder="+94 77 123 4567" /><FormField label="Daily rate (LKR)" type="number" value={driverForm.dailyRate} onChange={(value) => setDriverForm({ ...driverForm, dailyRate: value })} placeholder="12000" /><FormField label="Vehicle type" value={driverForm.vehicleType} onChange={(value) => setDriverForm({ ...driverForm, vehicleType: value })} placeholder="Family Van" /><FormField label="Vehicle model" value={driverForm.vehicleModel} onChange={(value) => setDriverForm({ ...driverForm, vehicleModel: value })} placeholder="Toyota KDH" /><FormField label="Vehicle capacity" type="number" value={driverForm.vehicleCapacity} onChange={(value) => setDriverForm({ ...driverForm, vehicleCapacity: value })} placeholder="7" /><FormField label="Experience (years)" type="number" value={driverForm.yearsOfExperience} onChange={(value) => setDriverForm({ ...driverForm, yearsOfExperience: value })} placeholder="5" /><FormField label="Languages" value={driverForm.languages} onChange={(value) => setDriverForm({ ...driverForm, languages: value })} placeholder="Sinhala, English" /><FormField label="Service areas" value={driverForm.serviceAreas} onChange={(value) => setDriverForm({ ...driverForm, serviceAreas: value })} placeholder="Ella, Kandy" /></div><label className="block"><span className="mb-2 block text-sm font-bold">Bio</span><textarea className="field min-h-24" maxLength="1000" value={driverForm.bio} onChange={(e) => setDriverForm({ ...driverForm, bio: e.target.value })} /></label><label className="block"><span className="mb-2 block text-sm font-bold">Verification</span><select className="field" value={driverForm.verificationStatus} onChange={(e) => setDriverForm({ ...driverForm, verificationStatus: e.target.value })}><option value="pending">Pending</option><option value="verified">Verified</option><option value="rejected">Rejected</option></select></label><label className="block"><span className="mb-2 block text-sm font-bold">Profile image {modal.id && <span className="font-normal text-slate-400">(optional when editing)</span>}</span><input className="field file:mr-4 file:rounded-full file:border-0 file:bg-teal-50 file:px-3 file:py-2 file:text-xs file:font-bold file:text-teal-700" type="file" accept="image/*" required={!modal.id} onChange={(e) => setDriverForm({ ...driverForm, image: e.target.files[0] })} /></label><label className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4"><input type="checkbox" checked={driverForm.availability} onChange={(e) => setDriverForm({ ...driverForm, availability: e.target.checked })} /><span><b className="block text-sm">Available for bookings</b><span className="text-xs text-slate-500">Customers can see this verified driver</span></span></label><SubmitButtons saving={saving} onCancel={() => setModal(null)} /></form></AdminModal>}
  </main>;
}

function Overview({ dashboard }) {
  const stats = [{ label: 'Destinations', value: dashboard?.counts.places || 0, icon: Map, color: 'bg-blue-50 text-blue-700' }, { label: 'Active drivers', value: dashboard?.counts.availableDrivers || 0, icon: Car, color: 'bg-emerald-50 text-emerald-700' }, { label: 'Travellers', value: dashboard?.counts.users || 0, icon: Users, color: 'bg-violet-50 text-violet-700' }, { label: 'Active requests', value: dashboard?.counts.activeBookings || 0, icon: CalendarDays, color: 'bg-amber-50 text-amber-700' }];
  return <div className="space-y-8"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{stats.map((item) => <article key={item.label} className="card flex items-center gap-4 p-5"><span className={`grid h-12 w-12 place-items-center rounded-2xl ${item.color}`}><item.icon size={22} /></span><div><p className="text-3xl font-bold">{item.value}</p><p className="text-sm text-slate-500">{item.label}</p></div></article>)}</div><section className="card p-6"><div className="mb-5"><p className="eyebrow">Schedule</p><h2 className="mt-2 font-serif text-2xl font-bold">Upcoming journeys</h2></div>{dashboard?.upcoming?.length ? <div className="divide-y divide-slate-100">{dashboard.upcoming.map((booking) => <div key={booking._id} className="grid gap-2 py-4 sm:grid-cols-[1fr_1fr_auto] sm:items-center"><div><b>{booking.destination}</b><p className="text-sm text-slate-500">{booking.user?.name}</p></div><p className="text-sm text-slate-600">{booking.driver?.fullName || booking.driver?.name}</p><time className="text-sm font-bold text-teal-700">{new Date(booking.startDate).toLocaleDateString(undefined, { timeZone: 'UTC' })}</time></div>)}</div> : <p className="py-8 text-center text-slate-500">No upcoming bookings yet.</p>}</section></div>;
}
...
function BookingsTable({ bookings }) { return <section><div className="mb-6"><h2 className="font-serif text-3xl font-bold">All bookings</h2><p className="text-sm text-slate-500">{bookings.length} total records</p></div><div className="card overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="p-4">Traveller</th><th className="p-4">Destination</th><th className="p-4">Driver</th><th className="p-4">Dates</th><th className="p-4">Price</th><th className="p-4">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{bookings.map((booking) => <tr key={booking._id}><td className="p-4"><b className="block">{booking.user?.name || booking.customerName}</b><span className="text-xs text-slate-500">{booking.user?.email}</span></td><td className="p-4 font-medium">{booking.destination}</td><td className="p-4">{booking.driver?.fullName || booking.driver?.name || 'Deleted driver'}</td><td className="p-4">{new Date(booking.startDate).toLocaleDateString(undefined, { timeZone: 'UTC' })} – {new Date(booking.endDate).toLocaleDateString(undefined, { timeZone: 'UTC' })}</td><td className="p-4">LKR {Number(booking.estimatedTotal || 0).toLocaleString()}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold capitalize ${['cancelled', 'rejected'].includes(booking.status) ? 'bg-slate-100 text-slate-600' : booking.status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>{booking.status}</span></td></tr>)}</tbody></table>{!bookings.length && <p className="p-12 text-center text-slate-500">No bookings yet.</p>}</div></section>; }
