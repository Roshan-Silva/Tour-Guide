...
export default function Home() {
  ...
  return <main>
    <section className="relative overflow-hidden bg-[#102f27] text-white">
      ...
      <div className="page-shell relative grid min-h-[650px] items-center gap-12 py-20 lg:grid-cols-[1.1fr_.9fr]">
        <div className="max-w-3xl">
          ...
          <div className="mt-9 flex flex-wrap gap-3"><Link to="/trip-planner" className="primary-button !bg-amber-300 !text-slate-950 hover:!bg-amber-200">Plan your journey <ArrowRight size={17} /></Link><Link to="/destinations" className="secondary-button !border-white/25 !bg-white/5 !text-white hover:!bg-white/10">Explore places</Link></div>
    ...
    <section id="destinations" className="page-shell py-24">
      ...
      {loading ? <div className="grid gap-6 md:grid-cols-3">{[1,2,3].map((item) => <div key={item} className="h-96 animate-pulse rounded-3xl bg-slate-200" />)}</div> : <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredPlaces.slice(0,6).map((place) => <Link to={`/destinations/${place.slug}`} key={place._id} className="card group overflow-hidden"><div className="h-64 overflow-hidden"><img src={getImageUrl(place.image)} alt={place.name} className="h-full w-full object-cover group-hover:scale-105 transition duration-500" /></div><div className="p-6"><div className="mb-3 flex items-center justify-between"><h3 className="font-serif text-2xl font-bold">{place.name}</h3><Waves size={19} className="text-teal-700" /></div><p className="mb-5 flex items-center gap-2 text-sm text-slate-500"><MapPin size={15} /> {place.location}</p><div className="flex flex-wrap gap-2">{(place.tags || []).map((tag) => <span key={tag} className="rounded-full bg-teal-50 px-3 py-1 text-xs font-bold capitalize text-teal-800">{tag}</span>)}</div></div></Link>)}</div>}
